...
void CEscenario::Leer_Mapa_unJugador(int** mapa, int n, CJugador* oJ) {
	...
	for (vector<CMejora*>::iterator it = Poderes.begin(); it != Poderes.end(); ++it) {
		delete *it;
	}

	for (vector<CEnemigo*>::iterator it = Enemigos.begin(); it != Enemigos.end(); ++it) {
		delete* it;
	}
	...
	for (int i = 0; i < Alto; ++i) {
		...
		for (int j = 0; j < Ancho; ++j) {
			getline(ss, aux, ',');
			mapa[i][j] = atoi(aux.c_str());//atoi->cadena a entero
		}
	...
	if (n < 11) {
		//Lee Enemigos
		getline(fn, aux);
		Numero_Enemigos = atoi(aux.c_str());
