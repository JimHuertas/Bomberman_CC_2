...
public:
	CMejora() {
		...
		i = rand() % 13 + 1; //del 1 al 13
		j = rand() % 15 + 1; //del 1 al 15
	...
	void dibujar(Graphics^ g, Bitmap^ bmpMejoras, int** matriz) {
		...
		while (ubicado == false) {
			...
			Rectangle aumento = Rectangle(j * 50, i * 50, ancho * 3.125, alto * 3.125); //tamaño distinto para que no se note la mejora
